...
// Inject fonts for Navy/Gold design
if (typeof document !== 'undefined' && !document.getElementById('casalapp-fonts')) {
  const link = document.createElement('link');
  link.id = 'casalapp-fonts';
  link.rel = 'stylesheet';
  link.href = 'https://fonts.googleapis.com/css2?family=Fraunces:opsz,wght@9..144,400;9..144,600;9..144,700&family=Plus+Jakarta+Sans:wght@400;500;600;700&display=swap';
...
const DK = {
  ...
  accentMid:"#C9A96E", success:"#5A9A6A", danger:"#D06060", warning:"#C9A96E",
};
    ...
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(15,18,24,0.55)',zIndex:4000,backdropFilter:'blur(8px)',display:'flex',alignItems:'center',justifyContent:'center'}}>
      <div onClick={e=>e.stopPropagation()} style={{background:T.card,border:'1px solid '+T.border,borderRadius:16,padding:28,width:400,maxHeight:'90vh',overflowY:'auto',boxShadow:'0 16px 60px rgba(30,24,16,0.2)',fontFamily:'inherit'}}>
        ...
        <div style={{display:'flex',gap:10,marginTop:22}}>
          ...
          <button onClick={()=>title&&date&&onSave({id:event?.id||genId(),title,date,type,desc,reminder,recur})} style={{padding:'9px 20px',background:'#1B2A4A',border:'none',borderRadius:10,color:'#fff',fontSize:13,cursor:'pointer',fontFamily:"'Plus Jakarta Sans',sans-serif"}}>{event?'Guardar':'Criar'}</button>
        </div>
      </div>
    </div>
  );
    ...
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(15,18,24,0.55)',zIndex:4000,backdropFilter:'blur(8px)',display:'flex',alignItems:'center',justifyContent:'center'}}>
      <div onClick={e=>e.stopPropagation()} style={{background:T.card,border:'1px solid '+T.border,borderRadius:16,padding:28,width:400,boxShadow:'0 16px 60px rgba(30,24,16,0.2)',fontFamily:'inherit'}}>
        ...
        <div style={{display:'flex',gap:10,marginTop:22}}>
          ...
          <div style={{flex:1}}/>
          <button onClick={onClose} style={{padding:'9px 16px',background:'none',border:'1px solid '+T.border,borderRadius:7,color:T.muted,fontSize:13,cursor:'pointer',fontFamily:'inherit'}}>Cancelar</button>
          <button onClick={()=>text&&onSave({id:task?.id||genId(),text,col,priority,done:col==='Concluído',priv,cat,dueDate,recur})} style={{padding:'9px 20px',background:'#1B2A4A',border:'none',borderRadius:10,color:'#fff',fontSize:13,cursor:'pointer',fontFamily:"'Plus Jakarta Sans',sans-serif"}}>{task?'Guardar':'Criar'}</button>
        </div>
      </div>
    </div>
  );
    ...
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(15,18,24,0.55)',zIndex:4000,backdropFilter:'blur(8px)',display:'flex',alignItems:'center',justifyContent:'center'}}>
      <div onClick={e=>e.stopPropagation()} style={{background:T.card,border:'1px solid '+T.border,borderRadius:18,width:440,maxHeight:'88vh',display:'flex',flexDirection:'column',boxShadow:'0 20px 70px rgba(30,24,16,0.22)',fontFamily:'inherit',overflow:'hidden'}}>
        ...
        <div style={{padding:'14px 24px',borderTop:'1px solid '+T.border,display:'flex',gap:10,flexShrink:0,background:T.card}}>
          <div style={{flex:1}}/>
          <button onClick={onClose} style={{padding:'9px 16px',background:'none',border:'1px solid '+T.border,borderRadius:7,color:T.muted,fontSize:13,cursor:'pointer',fontFamily:'inherit'}}>Cancelar</button>
          <button onClick={()=>onSave({name,photo,email,notif,notifEvents,notifTasks,notifChat,defaultAvail})} style={{padding:'9px 20px',background:'#1B2A4A',border:'none',borderRadius:10,color:'#fff',fontSize:13,cursor:'pointer',fontFamily:"'Plus Jakarta Sans',sans-serif"}}>Guardar</button>
        </div>
      </div>
    </div>
  );
        ...
        <div style={{marginTop:20,paddingTop:16,borderTop:'1px solid '+T.border,display:'flex',flexDirection:'column',gap:8}}>
          {!confirmDel ? (<>
            <button onClick={()=>setConfDel('leave')}
              style={{width:'100%',padding:'10px',background:'none',border:'1px solid '+T.border,borderRadius:10,color:T.muted,fontSize:13,cursor:'pointer',fontFamily:'inherit'}}>
              🚪 Sair do grupo
            </button>
            <button onClick={()=>setConfDel('delete')}
              style={{width:'100%',padding:'10px',background:'none',border:'1px solid '+T.danger,borderRadius:10,color:T.danger,fontSize:13,cursor:'pointer',fontFamily:'inherit'}}>
            ...
            <div style={{background:'#fee2e2',border:'1px solid #fca5a5',borderRadius:12,padding:'14px'}}>
              ...
              <div style={{fontSize:11,color:'#991b1b',marginBottom:12}}>{confirmDel==='leave'?'Vais sair. Os outros membros mantêm o grupo.':'Apaga o grupo e todos os dados para todos. Irreversível.'}</div>
              <div style={{display:'flex',gap:8}}>
                <button onClick={()=>setConfDel(false)}
                  style={{flex:1,padding:'8px',background:'none',border:'1px solid #fca5a5',borderRadius:8,color:'#991b1b',fontSize:12,cursor:'pointer',fontFamily:'inherit'}}>
                  Cancelar
                </button>
                <button onClick={()=>{ confirmDel==='leave'?(onLeave&&onLeave(group.id)):(onDelete&&onDelete(group.id)); onClose&&onClose(); }}
                  style={{flex:1,padding:'8px',background:'#dc2626',border:'none',borderRadius:8,color:'#fff',fontSize:12,cursor:'pointer',fontFamily:'inherit'}}>
      ...
      {open&&(
        <div style={{position:'absolute',bottom:54,right:0,width:300,background:T.card,border:'1px solid '+T.border,borderRadius:16,boxShadow:'0 10px 50px rgba(30,24,16,0.22)',overflow:'hidden',display:'flex',flexDirection:'column'}}>
          ...
          <div style={{padding:'9px 11px',borderTop:'1px solid '+T.border,display:'flex',gap:7,flexShrink:0}}>
            ...
            <button onClick={send} style={{padding:'7px 12px',background:'#1B2A4A',border:'none',borderRadius:10,color:'#fff',fontSize:13,cursor:'pointer',fontFamily:"'Plus Jakarta Sans',sans-serif"}}>→</button>
          </div>
        </div>
      )}
...
function DashboardView({ events, tasks, msgs, notes, profile, proposals, groups, onApprove, onReject, currentUserId }) {
  ...
  return (
    <div style={{padding:24,overflowY:'auto',height:'100%',boxSizing:'border-box'}}>
      <div style={{marginBottom:20}}>
        <div style={{fontSize:22,fontWeight:600,color:T.text,marginBottom:4,fontFamily:"'Fraunces',serif"}}>{greeting}, {profile.name||'Tu'} 👋</div>
    ...
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(30,24,16,0.5)',zIndex:4000,display:'flex',alignItems:'center',justifyContent:'center'}}>
      <div onClick={e=>e.stopPropagation()} style={{background:T.card,border:'1px solid '+T.border,borderRadius:16,padding:28,width:440,boxShadow:'0 16px 60px rgba(30,24,16,0.25)',fontFamily:'inherit'}}>
        ...
        <div style={{display:'flex',gap:10,justifyContent:'flex-end'}}>
          ...
          <button onClick={()=>onSave(sh)} style={{padding:'8px 18px',background:'#1B2A4A',border:'none',borderRadius:10,color:'#fff',fontSize:13,cursor:'pointer',fontFamily:"'Plus Jakarta Sans',sans-serif"}}>Guardar</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(30,24,16,0.5)',zIndex:4000,display:'flex',alignItems:'center',justifyContent:'center'}}>
      <div onClick={e=>e.stopPropagation()} style={{background:T.card,border:'1px solid '+T.border,borderRadius:16,padding:28,width:380,boxShadow:'0 16px 60px rgba(30,24,16,0.25)',fontFamily:'inherit'}}>
        ...
        <div style={{display:'flex',gap:10,justifyContent:'flex-end'}}>
          <button onClick={onClose} style={{padding:'9px 16px',background:'none',border:'1px solid '+T.border,borderRadius:7,color:T.muted,fontSize:13,cursor:'pointer',fontFamily:'inherit'}}>Cancelar</button>
          <button onClick={()=>onSend(target,msg)} style={{padding:'9px 20px',background:'#1B2A4A',border:'none',borderRadius:10,color:'#fff',fontSize:13,cursor:'pointer',fontFamily:"'Plus Jakarta Sans',sans-serif"}}>Propor →</button>
        </div>
      </div>
    </div>
  );
...
export default function App({ user, onLogout }) {
  ...
  const leaveGroup = async (gid) => {
    ...
    await supabase.from('user_groups').delete().eq('group_id',gid).eq('user_id',u.id);
    setGroups(gs=>{const updated=gs.filter(g=>g.id!==gid);lsSave('groups',updated);return updated;});
    ...
    <div style={{display:'flex',height:'100vh',fontFamily:"'Georgia','Times New Roman',serif",background:T.bg,overflow:'hidden'}}>
      ...
      <div style={{width:198,background:T.sidebar,borderRight:'1px solid '+T.sidebarBorder,display:'flex',flexDirection:'column',flexShrink:0,zIndex:1}}>
        <div style={{padding:'13px 14px',borderBottom:'1px solid '+T.sidebarBorder,display:'flex',alignItems:'center',gap:10}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:13,color:T.text,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',fontFamily:"'Fraunces',serif",fontWeight:600}}>{curGroup?.name}</div>
  ...
  lsSave('profile',p);
      ...
      {groupModal&&<GroupModal group={groupModal} onClose={()=>setGM(null)} onUpdate={gr=>{setGroups(gs=>gs.map(g=>g.id===gr.id?gr:g));setGM(gr);}} onJoinGroup={grp=>{joinGroup(grp);setGM(null);}} onDelete={deleteGroup} onLeave={leaveGroup} onDeleteAll={deleteGroupForAll}/>}
  ...
  const handle=async()=>{
    ...
    try{
      let r;
      if(mode==='login') r=await supabase.auth.signInWithPassword({email,password:pw});
      else{
        r=await supabase.auth.signUp({email,password:pw});
        if(!r.error&&r.data.user)
          await supabase.from('profiles').upsert({id:r.data.user.id,email,name:email.split('@')[0]});
      }
      if(r.error) setErr(r.error.message);
      else if(r.data.user) onLogin(r.data.user);
    }catch{setErr('Erro de ligação.');}
    setLoading(false);
  };
  return(
    <div onKeyDown={e=>e.key==='Enter'&&handle()} tabIndex={-1} style={{minHeight:'100vh',background:'linear-gradient(155deg,#0C1525 0%,#1B2A4A 40%,#141F38 70%,#0A1020 100%)',display:'flex',alignItems:'center',justifyContent:'center',padding:16,position:'relative',overflow:'hidden',fontFamily:"'Plus Jakarta Sans',sans-serif",outline:'none'}}>
      ...
      <div style={{width:'100%',maxWidth:380,position:'relative',zIndex:10}}>
        <div style={{textAlign:'center',marginBottom:36}}>
          <div style={{width:72,height:72,borderRadius:24,background:'linear-gradient(135deg,rgba(201,169,110,0.2),rgba(201,169,110,0.05))',border:'1px solid rgba(201,169,110,0.25)',display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 20px',boxShadow:'0 0 40px rgba(201,169,110,0.15)'}}>
            <span style={{fontSize:32,fontFamily:"'Fraunces',serif",color:'#C9A96E',fontWeight:700}}>C</span>
          </div>
          <h1 style={{fontSize:38,fontWeight:700,color:'#fff',fontFamily:"'Fraunces',serif",letterSpacing:'-0.02em',margin:0}}>CasalApp</h1>
          <div style={{width:56,height:1,background:'linear-gradient(90deg,transparent,#C9A96E,transparent)',margin:'12px auto'}}/>
          <p style={{fontSize:11,color:'rgba(255,255,255,0.3)',letterSpacing:'0.3em',textTransform:'uppercase',margin:0}}>O vosso espaço</p>
        </div>
        <div style={{borderRadius:32,padding:36,background:'rgba(30,42,74,0.5)',backdropFilter:'blur(60px)',border:'1px solid rgba(201,169,110,0.1)'}}>
          <div style={{display:'flex',gap:10,marginBottom:32}}>
            {[['login','Entrar'],['register','Registar']].map(([m,l])=>(
              <button key={m} onClick={()=>{setMode(m);setErr('');}} style={{flex:1,padding:'12px',borderRadius:14,fontSize:14,fontWeight:700,cursor:'pointer',border:'none',fontFamily:'inherit',transition:'all 0.3s',background:mode===m?'linear-gradient(135deg,#C9A96E,#D4B87E)':'rgba(255,255,255,0.05)',color:mode===m?'#0C1525':'rgba(255,255,255,0.5)',transform:mode===m?'translateY(-1px)':'none'}}>{l}</button>
            ))}
          </div>
          <div style={{display:'flex',flexDirection:'column',gap:18}}>
            <div>
              ...
              <input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="tu@email.com" style={{width:'100%',boxSizing:'border-box',padding:'14px 16px',borderRadius:14,background:'rgba(255,255,255,0.08)',border:'none',color:'#fff',fontSize:14,fontFamily:'inherit',outline:'none'}} onFocus={e=>e.target.style.background='rgba(255,255,255,0.13)'} onBlur={e=>e.target.style.background='rgba(255,255,255,0.08)'}/>
            </div>
            <div>
              ...
              <input type="password" value={pw} onChange={e=>setPw(e.target.value)} placeholder="••••••••" style={{width:'100%',boxSizing:'border-box',padding:'14px 16px',borderRadius:14,background:'rgba(255,255,255,0.08)',border:'none',color:'#fff',fontSize:14,fontFamily:'inherit',outline:'none'}} onFocus={e=>e.target.style.background='rgba(255,255,255,0.13)'} onBlur={e=>e.target.style.background='rgba(255,255,255,0.08)'}/>
            </div>
          </div>
          {err&&<div style={{marginTop:14,padding:'12px 14px',borderRadius:14,background:'rgba(239,68,68,0.15)',color:'#fca5a5',fontSize:13}}>{err}</div>}
          <button onClick={handle} disabled={loading} style={{width:'100%',marginTop:24,padding:'14px',borderRadius:14,background:'linear-gradient(135deg,#C9A96E,#D4B87E)',border:'none',color:'#0C1525',fontSize:15,fontWeight:700,cursor:'pointer',fontFamily:'inherit',boxShadow:'0 8px 24px rgba(201,169,110,0.25)',opacity:loading?0.7:1,transition:'all 0.2s'}}>
            {loading?'A processar...':(mode==='login'?'Entrar':'Criar Conta')}
          </button>
          <p style={{textAlign:'center',fontSize:11,color:'rgba(255,255,255,0.25)',marginTop:20,marginBottom:0}}>🔒 Privado e seguro. Apenas vós têm acesso.</p>
        </div>
      </div>
    </div>
  );
