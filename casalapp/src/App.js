...
if(typeof document!=='undefined'&&!document.getElementById('cf')){const lk=document.createElement('link');lk.id='cf';lk.rel='stylesheet';lk.href='https://fonts.googleapis.com/css2?family=Fraunces:opsz,wght@9..144,400;9..144,600;9..144,700&family=Plus+Jakarta+Sans:wght@400;500;600;700&display=swap';document.head.appendChild(lk);}
    ...
    <div>
      ...
      {!group.isDefault && (
        <div style={{marginTop:20,paddingTop:16,borderTop:'1px solid '+T.border,display:'flex',flexDirection:'column',gap:8}}>
          {!confirmDel ? (<>
            <button onClick={()=>setConfDel('leave')} style={{width:'100%',padding:'10px',background:'none',border:'1px solid '+T.border,borderRadius:10,color:T.muted,fontSize:13,cursor:'pointer',fontFamily:'inherit'}}>
              🚪 Sair do grupo
            </button>
            <button onClick={()=>setConfDel('delete')} style={{width:'100%',padding:'10px',background:'none',border:'1px solid '+T.danger,borderRadius:10,color:T.danger,fontSize:13,cursor:'pointer',fontFamily:'inherit'}}>
            ...
            <div style={{background:'#fee2e2',border:'1px solid #fca5a5',borderRadius:12,padding:'14px'}}>
              ...
              <div style={{display:'flex',gap:8}}>
                ...
                <button onClick={()=>{confirmDel==='leave'?(onLeave&&onLeave(group.id)):(onDeleteAll&&onDeleteAll(group.id));onClose&&onClose();}} style={{flex:1,padding:'8px',background:'#dc2626',border:'none',borderRadius:8,color:'#fff',fontSize:12,cursor:'pointer',fontFamily:'inherit'}}>
