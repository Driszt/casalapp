...
function GroupModal({group,onClose,onUpdate}){
  ...
  const generateInvite = async () => {
    ...
    try {
      ...
      // In real app, save to Supabase invites table
      // For now generate a link with the invite code
      const link = window.location.origin + '?invite=' + inviteId + '&group=' + group.id + '&name=' + encodeURIComponent(group.name) + '&emoji=' + encodeURIComponent(group.emoji);
      setLink(link);
      // Save invite to localStorage so the app can detect it
      const invites = JSON.parse(localStorage.getItem('pendingInvites')||'{}');
      invites[inviteId] = { groupId: group.id, groupName: group.name, groupEmoji: group.emoji, createdAt: new Date().toISOString() };
...
export default function App({ user, onLogout }) {
  ...
  const proposeAction = (type,item,action)=>{
    ...
    setProps(p=>[...p,{id:genId(),type,item,action,original,groupId:g,from:profile.name||'Tu',status:'pending',createdAt:new Date().toISOString()}]);
  };
  ...
  const approveProposal = id=>{
    ...
    if(p){
      if(p.type==='task'&&p.action==='add')      setTasks(e=>({...e,[p.groupId]:[...(e[p.groupId]||[]),{...p.item,id:genId()}]}));
      if(p.type==='event'&&p.action==='add')     setEvents(e=>({...e,[p.groupId]:[...(e[p.groupId]||[]).filter(x=>x.id!==p.item.id),p.item]}));
      if(p.type==='event'&&p.action==='edit')    setEvents(e=>({...e,[p.groupId]:(e[p.groupId]||[]).map(x=>x.id===p.item.id?p.item:x)}));
      if(p.type==='event'&&p.action==='delete')  setEvents(e=>({...e,[p.groupId]:(e[p.groupId]||[]).filter(x=>x.id!==p.item.id)}));
    }
    setProps(ps=>ps.map(x=>x.id===id?{...x,status:'approved'}:x));
  };
  ...
  return(
    <div style={{display:'flex',height:'100vh',fontFamily:"'Georgia','Times New Roman',serif",background:T.bg,overflow:'hidden'}}>
      ...
      {groupModal&&<GroupModal group={groupModal} onClose={()=>setGM(null)} onUpdate={gr=>{setGroups(gs=>gs.map(g=>g.id===gr.id?gr:g));setGM(gr);}}/>}
